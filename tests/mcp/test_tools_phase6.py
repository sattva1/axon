...
class TestHandleDeadCode:
    def test_no_repo_path_no_warning(self, mock_storage: MagicMock) -> None:
        """handle_dead_code without repo_path never prefixes a warning."""
        result = handle_dead_code(mock_storage, repo_path=None)
        assert 'axon analyze' not in result

    def test_stale_repo_warning_present(
        self, mock_storage: MagicMock, stale_repo: Path
    ) -> None:
        """Stale dead-code analysis causes warning to be prepended."""
        result = handle_dead_code(mock_storage, repo_path=stale_repo)
        assert 'axon analyze' in result

    def test_fresh_repo_no_warning(
        self, mock_storage: MagicMock, fresh_repo: Path
    ) -> None:
        """Fresh meta produces no warning prefix."""
        result = handle_dead_code(mock_storage, repo_path=fresh_repo)
        assert 'axon analyze' not in result
    ...
    def test_missing_meta_no_warning(
        ...
        result = handle_dead_code(mock_storage, repo_path=tmp_path)
        assert 'axon analyze' not in result
...
class TestHandleCommunities:
    def test_no_repo_path_no_warning(self, mock_storage: MagicMock) -> None:
        ...
        result = handle_communities(mock_storage, repo_path=None)
        assert 'axon analyze' not in result
    ...
    def test_stale_repo_warning_present(
        ...
        result = handle_communities(mock_storage, repo_path=stale_repo)
        assert 'axon analyze' in result
    ...
    def test_fresh_repo_no_warning(
        ...
        result = handle_communities(mock_storage, repo_path=fresh_repo)
        assert 'axon analyze' not in result
    ...
    def test_community_arg_honored_with_stale_repo(
        ...
        result = handle_communities(
            mock_storage, community='my-community', repo_path=stale_repo
        )
        # Warning prefix is present because the repo is stale.
        assert 'axon analyze' in result
    ...
    def test_uses_communities_field_not_dead_code(
        ...
        communities_result = handle_communities(
            mock_storage, repo_path=tmp_path
        )
        dead_code_result = handle_dead_code(mock_storage, repo_path=tmp_path)
...
class TestHandleReviewRisk:
    def test_no_repo_path_no_warning(self, mock_storage: MagicMock) -> None:
        ...
        result = handle_review_risk(mock_storage, diff='', repo_path=None)
        assert 'axon analyze' not in result
    ...
    def test_stale_repo_warning_present(
        ...
        result = handle_review_risk(
            mock_storage, diff='', repo_path=stale_repo
        )
        assert 'axon analyze' in result
    ...
    def test_fresh_repo_no_warning(
        ...
        result = handle_review_risk(
            mock_storage, diff='', repo_path=fresh_repo
        )
        assert 'axon analyze' not in result
    ...
    def test_uses_dead_code_field_not_communities(
        ...
        review_result = handle_review_risk(
            mock_storage, diff='', repo_path=tmp_path
        )
        communities_result = handle_communities(
            mock_storage, repo_path=tmp_path
        )
...
class TestDispatchToolRepoPath:
    def test_repo_path_flows_to_dead_code(
        ...
        """_dispatch_tool passes repo_path to handle_dead_code."""
        result = _dispatch_tool(
            'axon_dead_code', {}, mock_storage, repo_path=stale_repo
        )
        assert 'axon analyze' in result
    ...
    def test_no_repo_path_no_warning_via_dispatch(
        ...
        result = _dispatch_tool(
            'axon_dead_code', {}, mock_storage, repo_path=None
        )
        assert 'axon analyze' not in result
    ...
    def test_repo_path_flows_to_communities(
        ...
        """_dispatch_tool passes repo_path to handle_communities."""
        result = _dispatch_tool(
            'axon_communities', {}, mock_storage, repo_path=stale_repo
        )
        assert 'axon analyze' in result
    ...
    def test_repo_path_flows_to_review_risk(
        ...
        """_dispatch_tool passes repo_path to handle_review_risk."""
        result = _dispatch_tool(
            'axon_review_risk',
            {'diff': ''},
            mock_storage,
            repo_path=stale_repo,
        )
        assert 'axon analyze' in result
